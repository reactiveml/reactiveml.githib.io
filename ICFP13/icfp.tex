...
\todo{Mettre syntaxe dans 5 et semantique avec la preuve}
...
%
Similarly, we can define another combinator that runs of its two inputs depending on a condition `c`:
\begin{lstlisting}
...
The analysis can also handle combinators. We define here a combinator that takes as inputs two processes `q1` and `q2` and runs them in parallel in a loop: \todo{Start from example of section...}
\begin{lstlisting}
let process par_comb q1 q2 =
  loop
    run q1 || run q2
