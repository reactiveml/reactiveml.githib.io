...
%
The solution to the latter is pretty simple: never use blocking functions inside cooperative threads. All the facilities mentioned earlier provide either I/O libraries compatible with cooperative scheduling or means to safely call blocking functions. See \cite{Marlow:2004} for an overview on how to implement such libraries.
...
The paper ends with some discussion and examples~(Section~\ref{sec:discussion}) and related work~(Section~\ref{sec:related_work}). The work presented here is implemented in the \rml compiler.\footnote{\url{http://www.reactiveml.org}}
...
% To solve this problem, the programmer must explicitly cooperate with the scheduler by calling the `pause` operator~(line~7), which awaits the next instant, at the end of the loop:
\begin{rmlcl}[5]
[...]
    then (emit s (); time := time');
    pause 
...
As ML functions are always instantaneous, they are reactive if and only if they terminate. %This is where the syntactic separation between functions and processes helps us: 
...
\inferrule%*[left=(Equiv)]
  ...
  { \tyju{\tyenv}{\eseq{e_1}{e_2}}{\rt_2}{ \effcon{(\effpar{\rk_1}{\effmin})}{\rk_2} } } 
% \\
% \inferrule
...
%
%We refer the reader to Appendix~\ref{sec:big_step_other} or~\cite{Mandel:2005} for a more formal presentation, including the definition of the signal environment $S$. 
...
\inferrule%[When]
  { \tyjus{\tyenv}{e_1}{\rt}{\rk} \\\\
    \tyjus{\tyenv}{e}{\rtsig{\rt_1}{\rt_2}}{\effany} }
  { \tyjus{\tyenv}{ \ewhen{e_1}{e} } {\rt} {\rk} } 
%
\and
%
\inferrule%[Loop]
...
Using a type-based analysis makes it easy to deal with cases of aliasing, like in the following example:\footnote{These examples can be downloaded or tried in the online toplevel available at \url{http://reactiveml.org/icfp13}.}
\begin{lstlisting}
let rec process p =
  let q = (fun x -> x) p in
  run q 
